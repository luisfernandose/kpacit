...
var FWDUVPIMA = function(
		...
		this.controller_do = parent.controller_do;
		this.isStopped = true;
		this.videoScreen_do = parent.videoScreen_do;
		this.isReady = false;
		...
		*/
		this.setUpIMA = function(){
			if(!self.adContainer){
				...
				parent.videoHolder_do.addChildAt(self, parent.videoHolder_do.getChildIndex(parent.dumyClick_do));
		...
		this.onAdsManagerLoaded = function(e){
			...
			if(parent.data.aom_bl) self.setVolume(0);
			
			self.isReady = true;
			if(parent.hider) parent.hider.stop();
			if(parent.data.autoPlay_bl || parent.data.aom_bl || parent.isThumbClick_bl && FWDUVPPassword.isCorect) self.play(e);
		}
		...
		this.onAdError = function(e){
			...
			parent.isIMA = false;
			self.stop();
			if(self.videoScreen_do.isSafeToBeControlled_bl || parent.data.autoPlay_bl || parent.isThumbClick_bl && FWDUVPPassword.isCorect){
				parent.videoPoster_do.hide(true);
				self.videoScreen_do.play();
				if(parent.hider) parent.hider.start();
			}
		}
		...
		this.onContentPauseRequested = function(e){
			...
			self.isLinear = e.getAd().isLinear();
			self.resizeAndPosition();
			parent.subtitle_do.stopToLoadSubtitle();
			parent.dumyClick_do.setX(-8000);
		...
		this.onContentResumeRequested = function(){
			...
			var curPlaylist = parent.data.playlist_ar[parent.id];
			if(curPlaylist.subtitleSource) parent.loadSubtitle(curPlaylist.subtitleSource[curPlaylist.subtitleSource.length - 1 - curPlaylist.startAtSubtitle]["source"]);
		...
		this.onContentProgress = function(e){
			var curTime = FWDUVPVideoScreen.formatTime(e.w.currentTime);
			var totalTime = FWDUVPVideoScreen.formatTime(e.w.duration);
			var percent = e.w.currentTime /e.w.duration;
			parent.videoScreenUpdateTimeHandler({curTime: curTime, totalTime:totalTime, seconds:e.w.currentTime, totalTimeInSeconds:e.w.duration});
			parent.videoScreenUpdateHandler({percent:percent});
		}
		
		this.onContentPlay = function(e){
			self.isPlaying = true;
			parent.videoScreenPlayHandler();
		}
		
		this.onContentPaused = function(e){
			self.isPlaying = false;
			parent.videoScreenPauseHandler();
		}
		...
		this.onAllAdsComplete = function(){
			if(self.hasPostRoll){
				...
				parent.videoScreenPlayCompleteHandler();
			}
		}
		...
		*/
		this.resizeAndPosition = function(){
			...
			if(parent.controller_do){
				if(!self.isLinear) offsetY = self.controller_do.stageHeight;
			}
		
			self.sW = parent.tempVidStageWidth;
			self.sH = parent.tempVidStageHeight;
		...
		*/
		this.setSource = function(source){
			...
			//if(parent.data.autoPlay_bl || parent.data.aom_bl || parent.isThumbClick_bl && FWDUVPPassword.isCorect) parent.videoPoster_do.hide(true);
			parent.videoScreenUpdateHandler({percent:0});
			parent.dumyClick_do.setX(-8000);
			if(self.controller_do){
				self.controller_do.disablePlaybackRateButton();
			}
			...
			adsRequest.linearAdSlotWidth = parent.tempVidStageWidth;
			adsRequest.linearAdSlotHeight = parent.tempVidStageHeight;
			adsRequest.nonLinearAdSlotWidth = parent.tempVidStageWidth;
			adsRequest.nonLinearAdSlotHeight = parent.tempVidStageHeight;
