(function ($) {
    ...
    $("body").on("change", "#categories", function (e) {
        ...
        $.get(
            "/admin/filters/get-by-category-id/" + category_id,
            function (result) {
                if (
                    result &&
                    typeof result.filters !== "undefined" &&
            ...
            if (errors && errors.errors) {
                Object.keys(errors.errors).forEach((key) => {
                    ...
                    if (key === "zoom-not-complete-alert") {
                        form.find(".js-zoom-not-complete-alert").removeClass(
                            "d-none"
                        );
                    } else {
    ...
    $("body").on("click", "#webinarAddPrerequisites", function (e) {
        ...
        add_prerequisites_modal = add_prerequisites_modal.replaceAll(
    ...
    $("body").on("click", "#webinarAddQuiz", function (e) {
        ...
        add_quiz_modal += "</div>";
        add_quiz_modal = add_quiz_modal.replaceAll(
            "quiz-select2",
            "quiz-select22"
        );
        ...
        $.post(
            ...
            function (result) {
                if (result && result.ticket) {
                    ...
                    edit_ticket_modal = edit_ticket_modal.replaceAll(
                        "/admin/tickets/store",
                        "/admin/tickets/" + ticket_id + "/update"
                    );
    ...
    $("body").on("click", ".edit-session", function (e) {
        ...
        $.post(
            "/admin/sessions/" + session_id + "/edit",
            edit_data,
            function (result) {
                if (result && result.session) {
                    ...
                    edit_session_modal = edit_session_modal.replaceAll(
                        "/admin/sessions/store",
                        "/admin/sessions/" + session_id + "/update"
                    );
                    const nameId = randomString();
                    edit_session_modal = edit_session_modal.replaceAll(
                        "record",
                            ...
                            Object.keys(session).forEach((key) => {
                                if (key === "session_api") {
                                    ...
                                    if (session[key] === "big_blue_button") {
                                        $modal
                                            .find(".js-moderator-secret")
                                            .removeClass("d-none");
                                        $modal
                                            .find(".js-ajax-moderator_secret")
                                            .prop("disabled", true);
                                    }
        ...
        $.post(
            ...
            function (result) {
                if (result && result.file) {
                    ...
                    edit_file_modal += "</div>";
                    edit_file_modal = edit_file_modal.replaceAll(
                        "/admin/files/store",
                        "/admin/files/" + file_id + "/update"
                    );
                                ...
                                if (key === "storage") {
                                    if (file[key] === "online") {
                                        ...
                                        $modal
                                            ...
                                            .addClass("d-none");
                                        $modal
                                            .find(".js-downloadable-file input")
                                            .prop("checked", false);
                                    } else {
                                        $modal
                                            .find(".local-input")
    ...
    $("body").on("click", ".edit-prerequisite", function (e) {
        ...
        $.post(
            "/admin/prerequisites/" + prerequisite_id + "/edit",
            edit_data,
            function (result) {
                if (result && result.prerequisite) {
                    const prerequisite = result.prerequisite;

                    let edit_prerequisite_modal =
                        '<div id="addPrerequisitesModal">';
                    edit_prerequisite_modal += $(
                        "#webinarPrerequisitesModal"
                    ).html();
                    edit_prerequisite_modal += "</div>";
                    edit_prerequisite_modal =
                        edit_prerequisite_modal.replaceAll(
                            "prerequisites-select",
                            "prerequisites-select2"
                        );
                    edit_prerequisite_modal =
                    ...
                    edit_prerequisite_modal =
                        edit_prerequisite_modal.replaceAll("str_", "");
                    ...
                    Swal.fire({
                        ...
                        width: "48rem",
                        onOpen: () => {
                            $(".prerequisites-select2").append(
                                '<option selected="selected" value="' +
                                    prerequisite.webinar_id +
                                    '">' +
                                    prerequisite.webinar_title +
                                    "</option>"
                            );

                            if (prerequisite.required === 1) {
                                $("#addPrerequisitesModal")
                                    .find('[name="required"]')
                                    .prop("checked", true);
                            }
    ...
    $("body").on("click", ".edit-faq", function (e) {
        ...
        $.post("/admin/faqs/" + faq_id + "/edit", edit_data, function (result) {
            if (result && result.faq) {
                ...
                edit_faq_modal += "</div>";
                edit_faq_modal = edit_faq_modal.replaceAll(
                    "/admin/faqs/store",
                    "/admin/faqs/" + faq_id + "/update"
                );
    ...
    $("body").on("click", ".edit-webinar-quiz", function (e) {
        ...
        $.post(
            "/admin/webinar-quiz/" + webinar_quiz_id + "/edit",
            edit_data,
            function (result) {
                if (result && result.webinarQuiz) {
                    ...
                    edit_webinar_quiz_modal =
                        edit_webinar_quiz_modal.replaceAll(
                            "/admin/webinar-quiz/store",
                            "/admin/webinar-quiz/" + webinar_quiz_id + "/update"
                        );
                    edit_webinar_quiz_modal =
                        edit_webinar_quiz_modal.replaceAll(
                            "quiz-select2",
    ...
    $("body").on("click", "#webinarAddTestLesson", function (e) {
        ...
        addTextLesson = addTextLesson.replaceAll(
            "attachments-select2",
            "attachments-select2-" + nameId
        );
                    ...
                    summernoteTarget.summernote({
                        ...
                        callbacks: {
                            onChange: function (contents, $editable) {
                                $modal
                                    .find(".js-hidden-content-summernote")
                                    .val(contents);
                            },
                        },
                    });
                        ...
                        onOpen: () => {
                            ...
                            Object.keys(testLesson).forEach((key) => {
                                if (key === "attachments") {
                                    if (
                                        testLesson[key] &&
                                        testLesson[key].length
                                    ) {
                                        Object.keys(testLesson[key]).forEach(
                                            (k) => {
                            ...
                            var summernoteTarget = $modal.find(
