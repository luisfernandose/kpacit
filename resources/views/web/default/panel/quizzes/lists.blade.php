                                    ...
                                    <select name="quiz_id" class="form-control select2"
                                        ...
                                        @foreach ($allQuizzesLists as $allQuiz)
                                            <option value="{{ $allQuiz->id }}"
                                                ...
                                                {{ $allQuiz->title . ' - ' . $allQuiz->webinar_title }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
        ...
        <div class="d-flex align-items-start align-items-md-center justify-content-between flex-column flex-md-row">
            ...
            <form action="/panel/quizzes" method="get" class="">
                <div
                    ...
                    <label class="mb-0 mr-10 cursor-pointer text-gray font-14 font-weight-500"
                        for="activeQuizzesSwitch">{{ trans('quiz.show_only_active_quizzes') }}</label>
                    <div class="custom-control custom-switch">
                        <input type="checkbox" name="active_quizzes" class="custom-control-input"
                            id="activeQuizzesSwitch" @if (request()->get('active_quizzes', null) == 'on') checked @endif>
            ...
            <div class="no-result default-no-result d-flex align-items-end justify-content-center flex-column">
                <div class="d-flex align-items-center flex-column text-center">
                    <a href="/panel/quizzes/new" class="btn btn-sm btn-primary">{{ trans('quiz.create_a_quiz') }}</a>
                </div>
            </div>
        </div>
                            ...
                            <table class="table text-center custom-table">
                                <thead>
                                    <tr>
                                        ...
                                        <th class="text-center">{{ trans('public.time') }} <span
                                                class="braces">({{ trans('public.min') }})</span></th>
                                ...
                                <tbody>
                                    ...
                                    @foreach ($quizzes as $quiz)
                                        <tr>
                                            ...
                                            <td class="text-center align-middle">{{ $quiz->quizQuestions->sum('grade') }}
                                            </td>
                                            <td class="text-center align-middle">{{ $quiz->pass_mark }}</td>
                                            <td class="text-center align-middle">
                                                <span
                                                    class="d-block">{{ $quiz->quizResults->pluck('user_id')->count() }}</span>

                                                @if (!empty($quiz->userSuccessRate) and $quiz->userSuccessRate > 0)
                                                    <span
                                                        class="font-12 text-primary d-block">{{ $quiz->userSuccessRate }}%
                                                        {{ trans('quiz.passed') }}</span>
                                                @endif
                                            </td>

                                            <td class="text-center align-middle">{{ trans('public.' . $quiz->status) }}
                                            </td>

                                            <td class="text-center align-middle">
                                                {{ dateTimeFormat($quiz->created_at, 'j F Y H:i') }}</td>
                                            <td class="text-center align-middle">
                                                <div class="btn-group dropdown table-actions">
                                                    ...
                                                    <div class="dropdown-menu font-weight-normal">
                                                        <a href="/panel/quizzes/{{ $quiz->id }}/edit"
