...
@section('content')
    ...
    <div class="collapse" id="collapseExample">
        <section class="mt-25>
            ...
            <div class="panel-section-card p-10 mt-20">
                                    ...
                                    <div class="col-12 col-lg-4">
                                        <div class="form-group">
                                            ...
                                            <select class="form-control" id="status" name="status">
                                                ...
                                                <option value="passed"
                                                    {{ request()->get('status') === 'passed' ? 'selected' : '' }}>
                                                    {{ trans('quiz.passed') }}</option>
                                                <option value="failed"
                                                    {{ request()->get('status') === 'failed' ? 'selected' : '' }}>
                                                    {{ trans('quiz.failed') }}</option>
                                                <option value="waiting"
                                                    {{ request()->get('status') === 'waiting' ? 'selected' : '' }}>
                                                    {{ trans('quiz.waiting') }}</option>
                                            </select>
                                        </div>
                                    </div>
                                </div>
                ...
                <div class="row">
                    <div class="col-6 col-md-3 mt-30 mt-md-0 d-flex align-items-center justify-content-center">
                        <div class="d-flex flex-column align-items-center text-center">
                            ...
                            <strong
                                class="font-25 text-dark-blue font-weight-bold mt-5">{{ $quizzesResultsCount }}</strong>
                    ...
                    <div class="col-6 col-md-3 mt-30 mt-md-0 d-flex align-items-center justify-content-center">
                        <div class="d-flex flex-column align-items-center text-center">
                            ...
                            <strong class="font-25 text-dark-blue font-weight-bold mt-5">{{ $passedCount }}</strong>
                    ...
                    <div
                        ...
                        <div class="d-flex flex-column align-items-center text-center">
                            ...
                            <strong class="font-25 text-dark-blue font-weight-bold mt-5">{{ $failedCount }}</strong>
                    ...
                    <div
                        ...
                        <div class="d-flex flex-column align-items-center text-center">
                            ...
                            <strong class="font-25 text-dark-blue font-weight-bold mt-5">{{ $waitingCount }}</strong>
        ...
        <div class="d-flex align-items-start align-items-md-center justify-content-between flex-column flex-md-row">
            ...
            <form action="" method="get">
                <div
                    ...
                    <label class="mb-0 mr-10 cursor-pointer font-14 text-gray font-weight-500"
                        for="onlyOpenQuizzesSwitch">{{ trans('quiz.show_only_open_results') }}</label>
                    <div class="custom-control custom-switch">
                        <input type="checkbox" name="open_results" @if (request()->get('open_results', '') == 'on') checked @endif
                            class="custom-control-input" id="onlyOpenQuizzesSwitch">
                                        ...
                                        <tr>
                                            <td class="text-left">
                                                <div class="user-inline-avatar d-flex align-items-center">
                                                    <div class="avatar">
                                                        <img src="{{ $result->quiz->creator->getAvatar() }}"
                                                            class="img-cover" alt="">
                                                    </div>
                                                    <div class=" ml-5">
                                                        <span
                                                            class="d-block">{{ $result->quiz->creator->full_name }}</span>
                                                        <span
                                                            class="mt-5 font-12 text-gray d-block">{{ $result->quiz->creator->email }}</span>
                                                    </div>
                                                </div>
                                            </td>
                                            <td class="text-left">
                                                ...
                                                <span
                                                    class="font-12 text-gray d-block">{{ $result->quiz->webinar->title }}</span>
                                            </td>
                                            ...
                                            <td class="align-middle">
                                                <span
                                                    class="d-block text-{{ $result->status == 'passed' ? 'primary' : ($result->status == 'waiting' ? 'warning' : 'danger') }}">
                                                    {{ trans('quiz.' . $result->status) }}
                                                </span>

                                                @if ($result->status == 'failed' and $result->can_try)
                                                    <span
                                                        class="d-block font-12 text-gray">{{ trans('quiz.quiz_chance_remained', ['count' => $result->count_can_try]) }}</span>
                                                @endif
                                            </td>

                                            <td class="align-middle">
                                                {{ dateTimeFormat($result->created_at, 'j F Y H:i') }}
                                            </td>

                                            <td class="align-middle text-right font-weight-normal">
                                                <div
                                                    class="btn-group dropdown table-actions table-actions-lg table-actions-lg">
                                                    ...
                                                    <div class="dropdown-menu">
                                                        @if (!$result->can_try and $result->status != 'waiting')
                                                            <a href="/panel/quizzes/{{ $result->id }}/result"
                                                                class="webinar-actions d-block mt-10">{{ trans('public.view_answers') }}</a>
                                                        @endif

                                                        @if ($result->status != 'passed')
                                                            @if ($result->can_try)
                                                                <a href="/panel/quizzes/{{ $result->quiz->id }}/start"
                                                                    class="webinar-actions d-block mt-10">{{ trans('public.try_again') }}</a>
                                                            @endif
                                                        @endif

                                                        <a href="{{ $result->quiz->webinar->getUrl() }}"
                                                            class="webinar-actions d-block mt-10">{{ trans('webinars.webinar_page') }}</a>
                                                    </div>
                                                </div>
                                            </td>
                                        </tr>
