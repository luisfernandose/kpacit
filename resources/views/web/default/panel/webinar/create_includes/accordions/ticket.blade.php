<li data-id="{{ !empty($ticket) ? $ticket->id : '' }}"
    ...
    <div class="d-flex align-items-center justify-content-between " role="tab"
        ...
        <div class="font-weight-bold text-dark-blue" href="#collapseTicket{{ !empty($ticket) ? $ticket->id : 'record' }}"
            aria-controls="collapseTicket{{ !empty($ticket) ? $ticket->id : 'record' }}" data-parent="#ticketsAccordion"
        ...
        <div class="d-flex align-items-center">
                ...
                <div class="btn-group dropdown table-actions mr-15 font-weight-normal">
                    ...
                    <div class="dropdown-menu">
                        <a href="/panel/tickets/{{ $ticket->id }}/delete"
                            class="delete-action btn btn-sm btn-transparent">{{ trans('public.delete') }}</a>
                    </div>
                </div>
            @endif --}}
            ...
            <i class="collapse-chevron-icon" data-feather="chevron-down" height="20"
                ...
                aria-controls="collapseTicket{{ !empty($ticket) ? $ticket->id : 'record' }}"
                data-parent="#ticketsAccordion" role="button" data-toggle="collapse" aria-expanded="true"></i>
        </div>
    </div>

    <div id="collapseTicket{{ !empty($ticket) ? $ticket->id : 'record' }}"
        aria-labelledby="ticket_{{ !empty($ticket) ? $ticket->id : 'record' }}"
        class=" collapse @if (empty($ticket)) show @endif" role="tabpanel">
            ...
            <div class="ticket-form"
                ...
                <div class="row">
                    ...
                    <div class="col-12 col-lg-6">
                        <div class="form-group">
                            <label class="input-label">{{ trans('public.discount') }} <span
                                    class="braces">(%)</span></label>
                            <input type="text" name="ajax[{{ !empty($ticket) ? $ticket->id : 'new' }}][discount]"
                                class="js-ajax-discount form-control"
                ...
                <div class="mt-30 d-flex align-items-center justify-content-between">
                    <button type="button"
                        class="js-save-ticket btn btn-sm btn-primary">{{ trans('public.save') }}</button>

                    {{-- @if (empty($ticket)) --}}
                    @if (!empty($ticket))
                        <a href="/panel/tickets/{{ $ticket->id }}/delete"
                            class="delete-action btn btn-sm btn-danger">{{ trans('public.delete') }}</a>
                        {{-- <button type="button"
