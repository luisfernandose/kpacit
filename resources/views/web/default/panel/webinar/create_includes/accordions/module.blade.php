    ...
    <div id="collapseModule{{ !empty($module) ? $module->id :'record' }}" aria-labelledby="module_{{ !empty($module) ? $module->id :'record' }}" class=" collapse @if(empty($module)) show @endif" role="tabpanel">
        <div class="panel-collapse text-gray">
            <div class="module-form" data-action="/panel/modules/{{ !empty($module) ? $module->id . '/update' : 'store' }}">
                ...
                <div class="accordion-content-wrapper mt-15" id="contentAccordion" role="tablist" aria-multiselectable="true">
                    ...
                    @if($webinar->content->where('module_id', $module["id"])->count() !=0)
                        @foreach($webinar->content->where('module_id', $module["id"]) as $content)
                            <li data-id="{{$content->id}}" class="accordion-row">
                                <div class="d-flex align-items-center justify-content-between" role="tab" id="content_{{ $content->id }}">
                                    <table class="table">
                                        <tr>
                                            <td style="width:40%">{{$content->getContentType()}}</td>
                                            <td style="width:40%">{{$content->getContentTitle()}}</td>
                                            <td style="width:50px">
                                                <div class="d-flex align-items-center">
                                                ...
                                                @if(!empty($content))
                                                    <div class="btn-group dropdown table-actions mr-15">
                                                        ...
                                                        <div class="dropdown-menu">
                                                            <button type="button" onclick='editContent({{$module["id"]}},{{$content->id}}, "{{$content->resource_type}}")' class="btn btn-sm btn-transparent">{{ trans('public.edit') }}</button><br>
