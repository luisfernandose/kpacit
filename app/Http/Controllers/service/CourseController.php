    ...
    public function getCoursesearch(Request $request){
       ...
       if ($validator->passes()) { 
        ...
        foreach ($courses as $keyS => $valueS) {
            ...
            $valueS->price=($valueS->type==0) ? '' : $valueS->price;
            $valueS->discount_price=($valueS->type==0) ? '' : $valueS->discount_price;
            if($course_count->ids!=''){
...
public function getCoursedetail(Request $request){
    ...
    if ($validator->passes()) {
        ...
        $status = 200;        
        $ratings=0;
        $valueS= Course::select('id','user_id','title','slug','short_detail','detail','requirement','price','discount_price','preview_type','video','url','preview_image','type')->where('id',$request->course_id)->where('status','1')->first();
        $duration=0;
        if($valueS) {
            ...
            if($valueS->preview_type=='video'){
                ...
                if($urlSample!=''){
                    if(strpos($urlSample, 'vimeo.com')!=''){
                        ...
                        $valueS->vimeo=$response_a['request']['files']['progressive'];
                    }
                }
            }
            $valueS->currency=$this->current();
            ...
            foreach ($coursechapters as $cKey => $cValue) {
                $courseclass = CourseClass::select('title','type','video','url')->where('course_id',$valueS->id)->where('coursechapter_id',$cValue->id)->get();
                foreach ($courseclass as $ccKey => $ccValue) {
                    $ccValue->preview_type=$ccValue->type;                
                    $ccValue->vimeo=[];
                    if($ccValue->type=='url'){


                        $urlSample=$ccValue->url;
                        if($urlSample!=''){
                            if(strpos($urlSample, 'vimeo.com')!=''){
                                $ccValue->preview_type='vimeo';
                                $vimeoID=explode("vimeo.com/video/", $urlSample)[1];
                                ...
                                $ccValue->vimeo=$response_a['request']['files']['progressive'];
                            }else{
                               $ccValue->url = str_replace("https://youtu.be/", "https://youtube.com/watch?v=", $ccValue->url);
                           }
                       }
                   }else{
...
public function getCoursedetailnew(Request $request){
    $status = 400; 
    $rules = array(
        'course_id' =>'required',
        'user_id' =>'required'
    ...
    if ($validator->passes()) {
        ...
        if($valueS) {
            ...
            if($valueS->preview_type=='video'){
                $valueS->sub_type='upload';
                $valueS->url = url('video/preview/'.$valueS->video);
            }else{
                $valueS->sub_type='url';
                $urlSample=$valueS->url;
                if($urlSample!=''){
                    if(strpos($urlSample, 'vimeo.com')!=''){
                     ...
                     $valueS->preview_type='video';
                     $valueS->url = str_replace("https://youtu.be/", "https://youtube.com/watch?v=", $valueS->url);
                 }
             }
         }
         $valueS->currency=$this->current();
        ...
        foreach ($coursechapters as $cKey => $cValue) {
            ...
            foreach ($courseclass as $ccKey => $ccValue) {
                ...
                $ccValue->vimeo=[];    
                if($ccValue->preview_type=='zip' && $ccValue->preview_type=='pdf' || $ccValue->preview_type=='word'){

                    $ccValue->sub_type=$ccValue->preview_type;
                    $ccValue->preview_type='document';
                    if($ccValue->preview_type=='zip'){
                        if($ccValue->zip!=''){                   
                            $ccValue->url = url('files/zip/'.$ccValue->zip);
                        }
                    }else if($ccValue->preview_type=='pdf'){
                        if($ccValue->pdf!=''){                   
                            $ccValue->url = url('files/pdf/'.$ccValue->pdf);
                        }
                    }else if($ccValue->preview_type=='word'){
                        if($ccValue->word!=''){                   
                            $ccValue->url = url('files/word/'.$ccValue->word);
                        }
                    }

                }else if($ccValue->preview_type=='video'){
                    if($ccValue->video!=''){
                        $ccValue->sub_type='upload';
                        $ccValue->url = url('video/class/'.$ccValue->video);
                    }else{
                        $ccValue->sub_type='url';
                        $urlSample=$ccValue->url;
                        if($urlSample!=''){
                            if(strpos($urlSample, 'vimeo.com')!=''){
                                $ccValue->sub_type='vimeo';
                                $vimeoID=explode("vimeo.com/video/", $urlSample)[1];
                                $url = "https://player.vimeo.com/video/".$vimeoID."/config";
                                $ch = curl_init();
                                curl_setopt($ch, CURLOPT_URL, $url);
               ...
               } else if($ccValue->preview_type=='image'){
                if($ccValue->image!=''){
                    $ccValue->sub_type='upload';                    
                    $ccValue->url = url('images/class/'.$ccValue->image);
                }else{
                    $ccValue->sub_type='url';
                }
            }        
...
public function getCourse(Request $request){
    ...
    foreach ($categoryDet as $keyC => $valueC) {
        ...
        if($courseDET>0){
            if($catCount==5){ break; }
            $courses= Course::select('id','title','preview_image','price','discount_price','type')->where('category_id',$valueC->id)->where('status','1')->orderByRaw("RAND()")->limit(10)->get();
            foreach ($courses as $keyS => $valueS) {
                $valueS->type=($valueS->type==1) ? 'paid' : 'free';
                $valueS->price=($valueS->type==0) ? '' : $valueS->price;
                $valueS->discount_price=($valueS->type==0) ? '' : $valueS->discount_price;
...
{  
    ...
    if ($validator->passes()) { 
        ...
        $courses= Course::select('courses.id','courses.title','courses.user_id','courses.preview_image','courses.price','courses.discount_price','courses.type')->join('wishlists','courses.id','=','wishlists.course_id')->where('wishlists.user_id',$_REQUEST['user_id'])->orderBy('wishlists.id','DESC')->skip($data['page'])->take($data['perPage'])->get();
...
{  
    ...
    if ($validator->passes()) { 
       ...
       if(isset($request->page) && $request->page)
        ...
        $data['page']   = (1-1)*$data['perPage'];
    $courses= Course::select('id','title','user_id','preview_image','price','discount_price','type')->where('category_id',$request->c_id)->where('status','1')->skip($data['page'])->take($data['perPage'])->get();
    $cid=array();
